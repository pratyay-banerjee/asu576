...
class BidirectionalAttentionFlow(Model):
    ...
    def __init__(self, vocab: Vocabulary,
                 ...
                 phrase_layer: Seq2SeqEncoder,
                 similarity_function: SimilarityFunction,
                 l_matrix_attention: LinearMatrixAttention1,
                 sa_matrix_attention: LinearMatrixAttention1,
                 modeling_layer: Seq2SeqEncoder,
                 span_end_encoder: Seq2SeqEncoder,
                 dropout: float = 0.2,
        ...
        self._sa_matrix_attention = sa_matrix_attention
        self._l_matrix_attention = l_matrix_attention
        
        encoding_dim = phrase_layer.get_output_dim()
        modeling_dim = modeling_layer.get_output_dim()
        selfattent_dim = encoding_dim
        
        #span_start_input_dim = encoding_dim * 4 + modeling_dim
        
        span_start_input_dim = encoding_dim * 4 + modeling_dim + 4*selfattent_dim
        
        self._span_start_predictor = TimeDistributed(torch.nn.Linear(span_start_input_dim, 1))

        span_end_encoding_dim = span_end_encoder.get_output_dim()
        #span_end_input_dim = encoding_dim * 4 + span_end_encoding_dim
        span_end_input_dim = encoding_dim * 4 + span_end_encoding_dim + 4*selfattent_dim

        self._span_end_predictor = TimeDistributed(torch.nn.Linear(span_end_input_dim, 1))

        # Bidaf has lots of layer dimensions which need to match up - these aren't necessarily
        # obvious from the configuration files, so we check here.
        check_dimensions_match(modeling_layer.get_input_dim(), 4 * encoding_dim + 4*selfattent_dim,
        ...
        check_dimensions_match(span_end_encoder.get_input_dim(), 4 * encoding_dim + 3 * modeling_dim +4*selfattent_dim,
    ...
    def forward(self,  # type: ignore
        ...
        sa_question_sim = self._dropout(self._sa_matrix_attention(device1,embedded_question,None,True))
        sa_question_att = util.masked_softmax(sa_question_sim.to(device1),question_mask_temp.to(device1))
        sa_encoded_question = util.weighted_sum(encoded_question.to(device1),sa_question_att.to(device1))
        
        sa_encoded_question = sa_encoded_question.to(device0)
        
        sa_passage_sim = self._dropout(self._sa_matrix_attention(device2,embedded_passage,None,True))
        sa_passage_att = util.masked_softmax(sa_passage_sim.to(device1),passage_mask.clone().unsqueeze_(1).to(device1))
        sa_encoded_passage = util.weighted_sum(encoded_passage.to(device1),sa_passage_att.to(device1))
        sa_encoded_passage = sa_encoded_passage.to(device0)
        #sa_encoded_passage = encoded_passage
        
        # Shape: (batch_size, passage_length, question_length)
        passage_question_similarity = self._matrix_attention(encoded_passage, encoded_question)
        sa_passage_question_similarity = self._l_matrix_attention(device1,sa_encoded_passage, sa_encoded_question,False)
        
        # Shape: (batch_size, passage_length, question_length)
        passage_question_attention = util.masked_softmax(passage_question_similarity, question_mask_temp)
        sa_passage_question_attention = util.masked_softmax(sa_passage_question_similarity.to(device0), question_mask_temp)
        
        # Shape: (batch_size, passage_length, encoding_dim)
        passage_question_vectors = util.weighted_sum(encoded_question, passage_question_attention)
        sa_passage_question_vectors = util.weighted_sum(sa_encoded_question.to(device0), sa_passage_question_attention)
        ...
        masked_similarity = util.replace_masked_values(passage_question_similarity,
                                                       question_mask,
                                                       -1e7)
        sa_masked_similarity = util.replace_masked_values(sa_passage_question_similarity.to(device0),question_mask,-1e7)
        
        # Shape: (batch_size, passage_length)
        question_passage_similarity = masked_similarity.max(dim=-1)[0].squeeze(-1)
        sa_question_passage_similarity = sa_masked_similarity.max(dim=-1)[0].squeeze(-1)
        
        # Shape: (batch_size, passage_length)
        question_passage_attention = util.masked_softmax(question_passage_similarity, passage_mask)
        sa_question_passage_attention = util.masked_softmax(sa_question_passage_similarity, passage_mask)

        # Shape: (batch_size, encoding_dim)
        question_passage_vector = util.weighted_sum(encoded_passage, question_passage_attention)
        sa_question_passage_vector = util.weighted_sum(sa_encoded_passage, sa_question_passage_attention)
        ...
        sa_tiled_question_passage_vector = sa_question_passage_vector.unsqueeze(1).expand(batch_size,passage_length,encoding_dim)
