...
class BidirectionalAttentionFlow(Model):
    ...
    def forward(self,  # type: ignore
        ...
        if span_start is not None:
            ...
            preds =(na_inv.type(torch.cuda.FloatTensor) * util.masked_log_softmax(span_start_logits.type(torch.cuda.FloatTensor),passage_mask.type(torch.cuda.FloatTensor))).type(torch.cuda.FloatTensor).max(1)
            print("PREDS:",preds)
            ...
            #y = (na_inv.type(torch.cuda.ByteTensor) * span_start.squeeze(-1).type(torch.cuda.ByteTensor)).type(torch.cuda.LongTensor)
            #y = torch.bmm(na_inv.type(torch.cuda.LongTensor),span_start.type(torch.cuda.LongTensor)).unsqueeze(1).type(torch.cuda.LongTensor)
            y = na_inv.type(torch.cuda.LongTensor)
            print("NA-INV:",na_inv,"PREDS:",preds,"Y:",y)
            loss = nll_loss(preds,y)
