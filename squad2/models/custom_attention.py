...
@MatrixAttention.register("linear1")
class LinearMatrixAttention1(MatrixAttention):
    """
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        if not self_att:
            tiled_matrix_1 = matrix_1.to(device0).unsqueeze(2).expand(matrix_1.size()[0],
                                                          matrix_1.size()[1],
                                                          matrix_2.size()[1],
                                                          matrix_1.size()[2])
            tiled_matrix_2 = matrix_2.to(device0).unsqueeze(1).expand(matrix_2.size()[0],
                                                          matrix_1.size()[1],
                                                          matrix_2.size()[1],
                                                          matrix_2.size()[2])
            dtm1 = tiled_matrix_1
            ...
            tmd1 = dtm1.to(device1)
            tmd2 = dtm2.to(device1)
            prod = tmd1 * tmd2
            diff = tmd1 - tmd2
            combined_tensors = torch.cat([tmd1,tmd2,prod,diff],dim=-1)
            dot_product = torch.matmul(combined_tensors.to(device3), self._weight_vector.to(device3))
            return self._activation(dot_product.to(device1) + self._bias.to(device1))
        else:
            tiled_matrix_1 = matrix_1.to(device0).unsqueeze(2).expand(matrix_1.size()[0],
                                                          matrix_1.size()[1],
                                                          matrix_1.size()[1],
                                                          matrix_1.size()[2])
            tmd1 = tiled_matrix_1.to(device0)
            prod = tmd1 * tmd1
            combined_tensors = torch.cat([tmd1.to(device2),prod.to(device2)],dim=-1)
            dot_product = torch.matmul(combined_tensors.to(device3), self._weight_vector.to(device3))
            return self._activation(dot_product.to(device1) + self._bias.to(device1))